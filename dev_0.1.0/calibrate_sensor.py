...
if __name__ == "__main__":
    ...
    try:
        ...
        for color, rgb in calibration_colors.items():
            ...
            print("Wait to read the color please.")
            lcd.text("Reading color...", line=1)
            lcd.text("Please wait.", line=2)
            ...
            for i in range(5):
                ...
                global_min = [min(global_min[i], color_freq[color]) for i, color in enumerate(["RED", "GREEN", "BLUE"])]
                global_max = [max(global_max[i], color_freq[color]) for i, color in enumerate(["RED", "GREEN", "BLUE"])]
